...
begin
  ...
  if Length (EditCriarSenha.Text) < 6 then
     begin
       MessageDlg ('Sua senha deverá ter no mínimo 06 caractéres!', mtInformation, [mbOK], 0);
     end
   else
   if (EditConfirmacaoSenha.Text) <> EditCriarSenha.Text then
     begin
       MessageDlg ('Suas senhas deverão ser iguais', mtinformation, [mbOK], 0);
     end
   else
    if (EditEmailConfirmacao.Text) <> EditEmail.Text then
      begin
        MessageDlg ('O seu endereço de email está incorreto', mtInformation, [mbOK], 0);
      end
   else
